...
import { height as SCREEN_H } from '../constants';
import BounceOnHover from '../BounceOnHover';

export const ADMIN_NAV_HEIGHT = 64;
const NAV_H = ADMIN_NAV_HEIGHT;

const TABS = [
  { label: 'Dashboard', to: 'Admin' },
  { label: 'Clients', to: 'AdminClients' },
  { label: 'Manage Payments', to: 'AdminPayments' },
  // { label: 'Activity', to: 'AdminActivity' },
];
...
export default function AdminHeader() {
  ...
  const sm = width < SCREEN_H; // breakpoint: width > height ⇒ desktop
  ...
  const handleTabPress = (tabTo) => {
    if (route?.name !== tabTo) navigation.navigate(tabTo);
  };
  ...
  const activeLabel =
    TABS.find((t) => t.to === route?.name)?.label || 'Dashboard';
  ...
  return (
    <>
      ...
      >
        <BlurView
          ...
          style={[
            styles.blurBar,
            {
              backgroundColor: pal.blurFallback,
        ...
        >
          ...
          >
            <View style={[styles.brandPill, { backgroundColor: pal.brandPillBg }]}>
              <Text style={[styles.brandTxt, { color: pal.brandPillText }]}>
                IAS Admin
              </Text>
            </View>
          </BounceOnHover>
          ...
          <View style={[styles.tabsRow, { display: sm ? 'none' : 'flex' }]}>
            {TABS.map((t) => {
              const isActive = route?.name === t.to;
              return (
                <TouchableOpacity
                  key={t.to}
                  onPress={() => handleTabPress(t.to)}
                  activeOpacity={0.9}
                  style={[
                    styles.tab,
                    isActive && [
          ...
          <View style={styles.rightRow}>
            {/* Activity shortcut (bell) */}
            {sm ? (
              <TouchableOpacity
                onPress={() => handleTabPress('AdminActivity')}
                style={[
                  styles.iconBtn,
                ...
                activeOpacity={0.9}
              >
                <Feather name="activity" size={16} color={pal.iconOnGhost} />
              </TouchableOpacity>
            ) : (
              <TouchableOpacity
                onPress={() => handleTabPress('AdminActivity')}
                ...
                activeOpacity={0.9}
              >
                <Feather name="activity" size={14} color={pal.iconOnGhost} />
                <Text style={[styles.alertTxt, { color: pal.iconOnGhost }]}>
                  Activity
                </Text>
              </TouchableOpacity>
            )}
            ...
            {/* User + Logout (hidden on small, moved to menu) */}
            {!sm && (
              <>
                <TouchableOpacity
                  style={[
      ...
      {/* IMPORTANT: no spacer here.
         Your screens already pad with ADMIN_NAV_HEIGHT. */}
        ...
        <View style={[styles.menuWrap, { top: NAV_H + 8 }]}>
          ...
          >
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.9}
            >
              <Text style={[styles.menuText, { color: pal.text }]}>
            ...
            <TouchableOpacity onPress={handleLogout} style={styles.menuItem} activeOpacity={0.9}>
              <Text style={[styles.menuText, { color: pal.text }]}>Log Out</Text>
              <Feather name="log-out" size={16} color={pal.text} />
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
